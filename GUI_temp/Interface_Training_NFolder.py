...
import DataPreprocess,SVM_training,DT,DT_NFolder,SVM_training_NFolder
...
# ######Choose folder values######
rows = Label(setting_up, text="Please choose number of folders :") \
...
def TrainingOnDT():
    # global w1,filenameForTrainingResult,filenameForPredictedResult,\
    #     DTClass_weight,DTMax_features,\
    #     DTCriterion,unknowRow,hamRow,spamRow,PercisionRow,recallRow
    flolderValue  = w1.get()
    print  DTClass_weight.get()
    percisions, recalls, averagePercision, averageRecall = \
        DT_NFolder.DT(DTCriterion.get(),
                                                   DTMax_features.get(),
                     DTClass_weight.get(),pickleFileName,flolderValue)
    ...
    i=0
    percisionAndRecalls=''
    for percision in  percisions:
        percisionAndRecalls = percisionAndRecalls+ 'Percision: ' + str(
            percision) + '\t' + 'Recall: ' + str(recalls[i]) + '\r\n'
        i=i+1


    strOnPercisionRow = "Average precision On spam is : " + str(
        averagePercision)
    strOnRecallRow = "Average Recall on spam is : " + str(averageRecall)
    # unknowRow.set(strOnUnknowRow)
    # hamRow.set(strOnHamRow)

    spamRow.set(percisionAndRecalls)
    ...
    if (filenameForTrainingResult == ''):
        ...
        try:
            ...
            # fileOnTrainingResult.write('\r\n')
            fileOnTrainingResult.write(percisionAndRecalls)
            fileOnTrainingResult.write('\r\n')
...
def TrainingOnSVM():
    # global w1,filenameForTrainingResult,filenameForPredictedResult,SVMLoss,\
    #     SVMClass_weight,SVMKernel,unknowRow,hamRow,spamRow,PercisionRow,recallRow
    # here for presentation all use liner kernel
    flolderValue = w1.get()
    percisions, recalls, averagePercision, averageRecall = SVM_training_NFolder.svm_training(
        SVMKernel.get(), SVMLoss.get(),
                                          SVMClass_weight.get(),pickleFileName,flolderValue)
